	   ...
	   ButtonPanel.SortButtonListener, MyCanvas.VisualizerProvider
{
	...
	private static final int CAPACITY = 10, FPS = 10;
	...
	//visualizer.createRandomArray(canvas.getWidth(), canvas.getHeight());
		...
		// labels
		capacityLabel = new JLabel("Số lượng");
	...
	fpsSlider.setForeground(ColorManager.BACKGROUND);
		...
		// statistics
		// elapsed time
		timeLabel = new JLabel("Tổng thời gian: 0 µs");
		timeLabel.setFont(new Font(null, Font.PLAIN, 15));
		timeLabel.setForeground(ColorManager.TEXT_GREEN);

		// comparisons
		compLabel = new JLabel("Số lần so sánh: 0");
		compLabel.setFont(new Font(null, Font.PLAIN, 15));
		compLabel.setForeground(ColorManager.TEXT_GREEN);

		// swapping
		swapLabel = new JLabel("Số lần Swaps: 0");
	...
	{
		timeLabel.setText("Tổng thời gian: " + (int)(elapsedTime/1000.0) + " µs");
		compLabel.setText("Số lần so sánh: " + comp);
		swapLabel.setText("Số lần Swaps: " + swapping);
	}
